...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    CNewAsset asset1("XSRASSET", CAmount(100), 8, 1, 0, "");
    ...
    CReissueAsset reissue1("XSRASSET", CAmount(1), 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
    ...
    BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("XSRASSET"), "Map Reissued Asset should contain the asset \"XSRASSET\"");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("XSRASSET", Params().GlobalBurnAddress())) == CAmount(101), "Reissued amount wasn't added to the previous total");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddresses.at("XSRASSET").count(Params().GlobalBurnAddress()), "Reissued address wasn't in the map");

    // Get the new asset data from the cache
    CNewAsset asset2;
    BOOST_CHECK_MESSAGE(cache.GetAssetIfExists("XSRASSET", asset2), "Failed to get the asset2");
    ...
    BOOST_CHECK_MESSAGE(asset2.strName == "XSRASSET", "Asset2: Asset name is wrong");
    ...
    undoBlockIPFS.emplace_back(std::make_pair("XSRASSET", ""));
    ...
    CNewAsset asset3;
    BOOST_CHECK_MESSAGE(cache.GetAssetIfExists("XSRASSET", asset3), "Failed to get the asset3");
    ...
    BOOST_CHECK_MESSAGE(asset3.strName == "XSRASSET", "Asset3: Asset name is wrong");
    ...
    BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("XSRASSET"), "Map of reissued data was removed, even though changes were made and not databased yet");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("XSRASSET", Params().GlobalBurnAddress())) == CAmount(100), "Assets total wasn't undone when reissuance was");
    }
