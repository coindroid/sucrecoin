...
static const std::regex SUCRECOIN_NAMES("^XSR|^SUCRECOIN|^SUCRECOIN|^SUCRECOINC0IN|^SUCRECOINCO1N|^SUCRECOINC01N");
...
bool IsRootNameValid(const std::string& name)
{
    return std::regex_match(name, ROOT_NAME_CHARACTERS)
        ...
        && !std::regex_match(name, SUCRECOIN_NAMES);
}
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(XSR_R); // r
    vchMessage.push_back(XSR_V); // v
    vchMessage.push_back(XSR_N); // n
    vchMessage.push_back(XSR_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_XSR_ASSET << vchMessage << OP_DROP;
}
...
{
    // Reissuing an Asset must contain at least 3 CTxOut( Sucrecoin Burn Tx, Any Number of other Outputs ..., Owner Asset Change Tx, Reissue Tx)
    if (vout.size() < 3)
        return false;
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Sucrecoin Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3)
        return false;
...
{
    // Check the first transaction and verify that the correct XSR Amount
...
bool IsScriptNewAsset(const CScript& scriptPubKey)
{
    if (scriptPubKey.size() > 39) {
        if (scriptPubKey[25] == OP_XSR_ASSET && scriptPubKey[27] == XSR_R && scriptPubKey[28] == XSR_V && scriptPubKey[29] == XSR_N && scriptPubKey[30] == XSR_Q) {
            return true;
        }
    }
...
bool IsScriptOwnerAsset(const CScript& scriptPubKey)
{
    if (scriptPubKey.size() > 30) {
        if (scriptPubKey[25] == OP_XSR_ASSET && scriptPubKey[27] == XSR_R && scriptPubKey[28] == XSR_V && scriptPubKey[29] == XSR_N && scriptPubKey[30] == XSR_O) {
            return true;
        }
    }
...
{
    if (scriptPubKey.size() > 39) {
        if (scriptPubKey[25] == OP_XSR_ASSET && scriptPubKey[27] == XSR_R && scriptPubKey[28] == XSR_V && scriptPubKey[29] == XSR_N && scriptPubKey[30] == XSR_R) {
            return true;
        }
    }
...
{
    if (scriptPubKey.size() > 30) {
        if (scriptPubKey[25] == OP_XSR_ASSET && scriptPubKey[27] == XSR_R && scriptPubKey[28] == XSR_V && scriptPubKey[29] == XSR_N && scriptPubKey[30] == XSR_T) {
            return true;
        }
    }
    ...
    if (passets) {
        for (auto item : passets->setPossiblyMineRemove) {
            ...
            if (vpwallets[0]->IsMine(item.txOut) == ISMINE_SPENDABLE) {
                if (!passets->TrySpendCoin(item.out, item.txOut)) // Boolean true means only change the in memory data. We will want to save at the same time that XSR coin saves its cache
        ...
        for (auto item : passets->setPossiblyMineAdd) {
            // If the CTxOut is mine add it to the list of unspent outpoints
            if (vpwallets[0]->IsMine(item.txOut) == ISMINE_SPENDABLE) {
                if (!passets->AddToMyUpspentOutPoints(item.assetName, item.out)) // Boolean true means only change the in memory data. We will want to save at the same time that XSR coin saves its cache
                    error("%s: Failed to add an asset I own to my Unspent Asset Database. asset %s",
                                 __func__, item.assetName);
            }
        }
...
bool CreateAssetTransaction(CWallet* pwallet, const CNewAsset& asset, const std::string& address, std::pair<int, std::string>& error, std::string& rvnChangeAddress, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (haveChangeAddress && !IsValidDestinationString(rvnChangeAddress)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, "Change Address isn't a valid XSR address");
        return false;
    }
    ...
    // Check to make sure the wallet has the XSR required by the burnAmount
...
bool CreateReissueAssetTransaction(CWallet* pwallet, const CReissueAsset& reissueAsset, const std::string& address, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sucrecoin address: ") + address);
        return false;
    }
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sucrecoin address: ") + change_address);
            return false;
        }
    } else {
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any XSR, transfering an asset requires a network fee"));
        return false;
    }
